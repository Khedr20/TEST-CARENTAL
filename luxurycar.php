...
class LuxuryCar
{
    ...
    public function getAllLuxuryCars()
    {
        $query = "SELECT * FROM cars WHERE category = 'luxury'";
        $results = mysqli_query($this->db->getConnection(), $query);
        return $results;
    }

    public function getExistingLuxuryReservation($username)
    {
        $query = "SELECT * FROM cars WHERE reserved_by = '$username' AND reserved = 1 AND category = 'luxury'";
        $result = mysqli_query($this->db->getConnection(), $query);
        return mysqli_num_rows($result) > 0;
    }

    public function reserveLuxuryCar($carId, $username, $reservationDate, $reservationTime, $duration)
    {
    ...
    public function unreserveLuxuryCar($carId, $username)
    {
    ...
    public function returnLuxuryCar($carId, $username)
    {
